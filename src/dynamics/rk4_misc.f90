...
subroutine copy_patch_init(sourcesite,ipa,targetp)
   ...
   call find_derived_thbounds(targetp%can_rhos,targetp%can_theta,targetp%can_temp          &
                             ...
                             ,targetp%can_depth,sourcesite%ntext_soil(:,ipa))!ATT
   ...
   call ed_grndvap8(ksn,targetp%soil_water(nzg),targetp%soil_tempk(nzg)                    &
                   ...
                   ,targetp%ground_fliq,targetp%ggsoil,sourcesite,ipa)!ATT
   !---------------------------------------------------------------------------------------!
...
subroutine update_diagnostic_vars(initp, csite,ipa)
   ...
   do k = rk4site%lsl, nzg
      soilhcap = soil8(csite%ntext_soil(k,ipa))%slcpd!ATT
   ...
   sfcwloop: do k=1,ksn
      ...
      elseif (initp%flag_sfcwater == 1) then
         ...
         hcapdry_tot = soil8(csite%ntext_soil(nzg,ipa))%slcpd * dslz8(nzg)!ATT
         !---------------------------------------------------------------------------------!
   ...
   if (ok_ground) then
      call ed_grndvap8(ksn,initp%soil_water(nzg),initp%soil_tempk(nzg)                     &
                      ...
                      ,initp%ground_fliq,initp%ggsoil,csite,ipa)!ATT
   end if
   !---------------------------------------------------------------------------------------!
...
subroutine adjust_sfcw_properties(nzg,nzs,initp,hdid,csite,ipa)
   ...
   nsoil     = csite%ntext_soil(nzg,ipa)!ATT
   !---------------------------------------------------------------------------------------!
...
subroutine adjust_topsoil_properties(initp,hdid,csite,ipa)
   ...
   kt            = nzg
   nstop         = csite%ntext_soil(kt,ipa)!ATT
   ...
   if (slightlydry) then
      ...
      !------------------------------------------------------------------------------------!
      kb              = nzg -1 
      nsbeneath       = csite%ntext_soil(kb,ipa)!ATT
...
subroutine print_csiteipa(csite, ipa)
   ...
   do k=rk4site%lsl,nzg
      write (unit=*,fmt='(i5,1x,i12,4(es12.4,1x))') k,csite%ntext_soil(k,ipa)                &!ATT
