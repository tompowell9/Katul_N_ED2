...
!------------------------------------------------------------------------------------------!
subroutine update_turnover(cpoly, isi, doy, year, lat)
   use ed_state_vars  , only : polygontype        & ! structure
                             , sitetype           & ! structure
                             , patchtype          ! ! structure
   ...
   use phenology_coms , only : rad_turnover_int   & ! intent(in)
                             ...
                             , vm_amp             & ! intent(in)
                             , vm_min             ! ! intent(in)
   use consts_coms    , only : day_sec, pi1, pio180! ! intent(in)
use consts_coms, only: pio180, pi1
   ...
   patchloop: do ipa = 1,csite%npatches
      ...
      ! ATT taken from phenology_jeong.f90
      ! Current declination angle used to calculate day length for 
      ! seasonal variation of photosynthesis in deciduous trees
      doy_use=doy
      ...
      cohortloop: do ico = 1,cpatch%ncohorts
         ...
         !ATT update vm_bar for aspen
        ! select case (ipft)
        ! case (21)
        !   cpatch%vm_bar(ico) = min(1.,(daylength/daylength_solstice)**2)
        ! end select
        ! if (ico ==1) then
       !     print*,'vmbar',cpatch%vm_bar(ico)
        ! end if
...
subroutine first_phenology(cgrid)
   ...
   polyloop: do ipy=1,cgrid%npolygons
      ...
      siteloop: do isi=1,cpoly%nsites
         csite => cpoly%site(isi)
         patchloop: do ipa=1,csite%npatches
            call initialize_paw(csite,ipa,csite%ntext_soil(:,ipa))!ATT
...
!   cpatch%paw_avg(ico) = cpatch%paw_avg(ico)/(-1.0*slz(cpatch%krdepth(ico)))
   ...
   case default
      ...
      cpatch%bstorage(ico) =  bleaf_max  * (1 + q(ipft)) * 1.0
    cpatch%nstorage(ico) =  cpatch%bstorage(ico)/c2n_leaf(ipft)
   end select
   cpatch%broot(ico)    = bleaf_max * q(ipft)
   cpatch%bsapwood(ico) = dbh2bs(cpatch%dbh(ico),cpatch%pft(ico))
