...
subroutine event_harvest(agb_frac8,bgb_frac8,fol_frac8,stor_frac8)
  ...
  real :: ialloc,bdead_new,bsw_new,bleaf_new,bfr_new,bstore_new,nstore_new !JL added nstore_new
        ...
        do isi = 1,cpoly%nsites
           ...
           do ipa=1,csite%npatches
              ...
              do ico=1,cpatch%ncohorts
                 ...
                 bsw_new   = cpatch%bsapwood(ico) *&
                        (1.0-agb_frac*agf_bs - bgb_frac*(1.0-agf_bs))
                 bstore_new = cpatch%bstorage(ico)*(1.0-stor_frac)
                 nstore_new = cpatch%nstorage(ico)*(1.0-stor_frac) !JL!
                 ...
                 cpatch%nstorage(ico) = max(0.0,nstore_new) !JL!
     ...
     do ipy = 1,cgrid%npolygons
        ...
        do isi = 1,cpoly%nsites
           ...
           do ipa=1,csite%npatches
              ...
              call plant_patch(csite,ipa,nzg,pft,density,csite%ntext_soil(:,ipa) &         !ATT
                              ,csite%green_leaf_factor(:,ipa),planting_ht,cpoly%lsl(isi))
              call update_patch_derived_props(csite, cpoly%lsl(isi), cpoly%met(isi)%prss,ipa)
              call new_patch_sfc_props(csite, ipa,nzg,nzs,csite%ntext_soil(:,ipa))         !ATT
              call update_budget(csite, cpoly%lsl(isi),ipa,ipa)
           ...
           do ipa=1,csite%npatches
              ...
              do ico=1,cpatch%ncohorts
                 ...
                 csite%slow_soil_C(ipa) = csite%slow_soil_C(ipa) +                   &
                           ...
                           * (1 - f_fast(pft))
                 csite%slow_soil_N(ipa) = csite%slow_soil_N(ipa)                     &
                 ...
                 csite%fast_soil_C(ipa) = csite%fast_soil_C(ipa) +                   &
                           (f_labile(pft)*cpatch%balive(ico) +                       &
                           cpatch%bstorage(ico)) * cpatch%nplant(ico) * f_fast(pft)  
                 csite%fast_soil_N(ipa) = csite%fast_soil_N(ipa)                     &
                           + (f_labile(pft)*cpatch%balive(ico)/c2n_leaf(pft)         &
                           + cpatch%nstorage(ico)) * cpatch%nplant(ico) * f_fast(pft)
                 ...
                 csite%structural_soil_L(ipa) = csite%structural_soil_L(ipa) +       &
                      ...
                      cpatch%bdead(ico)* l2n_stem / c2n_stem(pft)) * cpatch%nplant(ico)
