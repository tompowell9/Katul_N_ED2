     ...
     do ipy=1,cgrid%npolygons
        ...
        do isi=1,cpoly%nsites
           csite => cpoly%site(isi)
              ...
              slsl=cpoly%lsl(isi)
              nsoil=cpoly%ntext_soil(slsl,isi)!ATT
           ...
           do ipa=1,csite%npatches
              ...
              do k = cpoly%lsl(isi),nzg
                 nsoil = csite%ntext_soil(k,ipa) !! mcd [9/30/08]  ATT
     ...
     do ipy=1,cgrid%npolygons
        ...
        if(soil_sat_water > 1.e-6 .and. fracliqtotal > FracLiqRunoff) then 
           ...
           do isi=1,cpoly%nsites
                 ...
                 !!*******************************************************************!!
                 cpoly%moist_tau(isi) = soil(cpoly%ntext_soil(nzg-1,isi))%slmsts / & !ATT
                 ...
                 !!*******************************************************************!!
                 cpoly%baseflow(isi) = soil(cpoly%ntext_soil(nzg-1,isi))%slmsts/ &!ATT
...
subroutine calcWatertable(cpoly,isi,ipa)
  ...
  layerloop: do k = cpoly%lsl(isi),nzg
     nsoil = csite%ntext_soil(k,ipa)  !look up soil type (switched to using SITE level soils [mcd 9/30/08]  ATT
...
subroutine updateWatertableAdd(cpoly,isi,ipa,dw,sheat)
  ...
  !!******************************************************************************!!
   layerloop: do k = cpoly%lsl(isi),nzg
      nsoil = csite%ntext_soil(k,ipa)  !look up soil type ATT
      fracw = csite%soil_water(k,ipa) / soil(nsoil)%slmsts  !calculate fraction of moisture capacity
      if(fracw < 1.0) then
...
subroutine updateWatertableBaseflow(cpoly,isi,ipa,baseflow)
   ...
   slsl  = cpoly%lsl(isi)
   nsoil = csite%ntext_soil(slsl,ipa)!ATT
