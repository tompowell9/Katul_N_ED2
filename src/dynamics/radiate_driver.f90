         ...
         siteloop: do isi = 1,cpoly%nsites
            ...
            !----- Get unnormalized radiative transfer information. -----------------------!
               call sfcrad_ed(cgrid%cosz(ipy),cpoly%cosaoi(isi),csite,nzg,nzs                 &
                          ,cpoly%ntext_soil(:,isi),maxcohort,tuco                          &!ATT OK
...
subroutine sfcrad_ed(cosz,cosaoi,csite,mzg,mzs,ntext_soil,maxcohort,tuco                   &
   ...
   do ipa = 1,csite%npatches
      ...
      !nsoil = ntext_soil(mzg)!ATT
      nsoil = csite%ntext_soil(mzg,ipa)!ATT

! by XXT
! from Liang et al. 2005 Development of land surface albedo parameterization
! based on Moderate Resolution Imaging Spectroradiometer (MODIS) data
! Journal of Geophysical Research

! In the paper, soil albedo is dependent on zenith angle and soil moisture
! However, for simplicity here, I only used the soil-moisture dependent part
! I also combined beam and diffuse albedo together to be consistent with the
! existing codes.

! Use parameters for broadleaf deciduous land cover
		albedo_soil_par = 0.295   *   &   ! background soil albedo
						((1 - 0.679) + & 
						 0.679 * exp(-14.44 * csite%soil_water(mzg,ipa))) !	correcting factor dependent on soil moisture

		albedo_soil_nir = 0.44   *   &   ! background soil albedo
						((1 - 0.640) + & 
						 0.640 * exp(-11.65 * csite%soil_water(mzg,ipa))) !	correcting factor dependent on soil moisture
