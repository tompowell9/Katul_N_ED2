...
subroutine canopy_photosynthesis(csite,cmet,mzg,ipa,lsl,ntext_soil                         &
   ...
   use katul_stomatal , only : katul_lphys
   implicit none
   !----- Arguments -----------------------------------------------------------------------!
   ...
   integer, parameter :: stoma_scheme = 2
   ...
   if (las) then
      ...
      do ipft = 1, n_pft
         if (include_pft(ipft)) then
            ...
            !if (tuco == 1)then
             !  print*,'vmbar',cpatch%vm_bar(tuco)
            !end if
            !------------------------------------------------------------------------------!
            ...
            ! patch area.                                                                  !
            !------------------------------------------------------------------------------!
			if(stoma_scheme == 2 ) then ! Katul Scheme, in this case stomatal_slope represents katul's lambda
				call katul_lphys(leaf_par * 4.6, & !uEn m-2 s-1
								 csite%can_co2(ipa),			 &  !umol CO2 / mol air
								 cpatch%leaf_temp(tuco) - t00, & ! leaf temperature in degC
								 ...
								 leaf_resp,&
								 cpatch%m_coef(tuco),&
								 cpatch%hite_coef(tuco), &
								 ipft,tuco, &
								 cpatch%former_gV(tuco), &
								 cpatch%former_gJ(tuco))
			else  ! Leuning's scheme
					            call lphysiol_full(            & !
   ...
   cohortloop: do ico = 1,cpatch%ncohorts
      ...
      if (cpatch%leaf_resolvable(ico)) then
			...
			if(stoma_scheme == 2 ) then ! Katul Scheme, in this case stomatal_slope represents katul's lambda
				call katul_lphys(leaf_par * 4.6, & !uEn m-2 s-1
								 csite%can_co2(ipa),			 &  !umol CO2 / mol air
								 cpatch%leaf_temp(ico) - t00, & ! leaf temperature in degC
								 ...
								 leaf_resp,&
								 cpatch%m_coef(ico),&
								 cpatch%hite_coef(ico), &
								 ipft,ico, &
								 cpatch%former_gV(ico), &
								 cpatch%former_gJ(ico))
			else  ! Leuning's scheme
					            call lphysiol_full(            & !
               							csite%can_prss(ipa)         & ! Canopy air pressure              [       Pa]
							          , csite%can_rhos(ipa)         & ! Canopy air density               [    kg/m³]
	...
	!if(ico ==	2)print*,'hour',current_time%hour,'leaf_psi',cpatch%psi_leaf(ico)
	if(isnan(cpatch%A_open(ico)))print*,'nan ico',ico,'nan leaf_gbw',cpatch%leaf_gbw(ico)
			...
			case (3)  ! leaf water potential scheme
				cpatch%fsw(ico) = 1.0
            ...
            cpatch%gpp(ico)       = cpatch%lai(ico)                                        &
                                  * ( cpatch%fs_open(ico) * cpatch%A_open(ico)             &
                                  ...
                                  + cpatch%leaf_respiration(ico)
!	if(ico == 1) print*,'fs_open',cpatch%fs_open(ico),'A_open',cpatch%A_open(ico),'gsw',cpatch%gsw_open(ico)
