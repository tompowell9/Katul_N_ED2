...
subroutine leaftw_derivs(mzg,mzs,initp,dinitp,csite,ipa)
   ...
   integer,  parameter :: 		water_extraction_scheme = 3
   !---------------------------------------------------------------------------------------!
   ...
   !---------------------------------------------------------------------------------------!
   select case (water_extraction_scheme)
   case (1)
   ...
   case (2,3) 
	! 1. not constrained by soilwp
	! 2. not cumulated values
	
	rk4aux%available_liquid_water(mzg) = dslz8(mzg)                                         &
    	                                  * max( 0.0d0                                         &
        	                              , initp%soil_fracliq(mzg)*initp%soil_water(mzg))
	do k = mzg, klsl, -1
	      rk4aux%available_liquid_water(k) = dslz8(k)     &
    	                                   * max( 0.0d0                                        &
        	                                    , initp%soil_fracliq(k) * initp%soil_water(k))
    end do
   endselect
   !---------------------------------------------------------------------------------------!
   ...
   !---------------------------------------------------------------------------------------!
   nsoil = csite%ntext_soil(klsl,ipa)!ATT
   select case (isoilbc)
   case (0)
      !------------------------------------------------------------------------------------!
   ...
   if (infiltration_method /= 0) then
      ...
      if (initp%virtual_water /= 0.d0) then  !!process "virtural water" pool
         nsoil = csite%ntext_soil(mzg,ipa)!ATT
         if (nsoil /= 13) then
   ...
   dinitp%avg_nutrient_layer_drainage      = - rk4aux%w_flux(nlsl) * wdns8
   dinitp%avg_drainage_heat = - rk4aux%qw_flux(klsl)
!	print*,'w_flux',rk4aux%w_flux(nlsl:mzg+1)
   ...
   case (1)
	...
	case(2,3)
	   if (any_resolvable) then
    	  do k1 = mzg, klsl, -1    ! loop over water layer
         	nsoil = csite%ntext_soil(k1,ipa)
            if (nsoil /= 13) then
            	if (rk4aux%available_liquid_water(k1) > 0.d0) then
                 	wloss = wdnsi8 * rk4aux%extracted_water(k1) * dslzi8(k1)                    ! m3/kg * kg/m2 / m
					dinitp%soil_water(k1) = dinitp%soil_water(k1) - dble(wloss)

                  	!----- Energy: only liquid water is lost through transpiration. ---------!
                  	qwloss = wloss * cliqvlme8 * (initp%soil_tempk(k1) - tsupercool8)
                  	dinitp%soil_energy(k1)   = dinitp%soil_energy(k1)   - qwloss
                	dinitp%avg_transloss(k1) = dinitp%avg_transloss(k1) - wdns8*dble(wloss)  &
                 	            * dslz8(k1)
                	endif
             	endif
      	  end do
...
!if(ico == 1) print*,'leaf_resolvable(ico)',initp%leaf_resolvable(ico)
...
!if(ico == 1) &
!	print*,'lint_shv',initp%lint_shv(ico),'initp%can_shv',initp%can_shv,'wflx_try',wflxlc_try,&
!			'initp%can_rhv',initp%can_rhv
...
!if(ico == 1)print*,'transp',transp,'fs_open',initp%fs_open(ico),'gsw_open',initp%gsw_open(ico),'gsw_closed',initp%gsw_closed(ico)
               else
		 ...
		 case (2)
		 	! distribute transp accordint to water_supply_layer_frac
			do k = mzg, 1, -1  ! circulate all the soil layer
			 rk4aux%extracted_water(k) = rk4aux%extracted_water(k) + &
			 		transp * cpatch%water_supply_layer_frac(k,ico)
			enddo
		 case (3)
			do k = mzg, 1, -1  ! circulate all the soil layer
			 rk4aux%extracted_water(k) = 0.
			enddo
		 endselect
         !---------------------------------------------------------------------------------!
   ...
   if (abs(dinitp%can_lntheta) > 10000) then
      print*,'hflxgc',hflxgc,'hflxlc_tot',hflxlc_tot,'hflxwc_tot',hflxwc_tot,'hflxac',hflxac,'hcapcani',hcapcani,'can_lntheta',dinitp%can_lntheta
   end if
