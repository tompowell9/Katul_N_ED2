...
subroutine reproduction(cgrid, month)
   ...
   polyloop: do ipy = 1,cgrid%npolygons
      ...
      !------------------------------------------------------------------------------------!
      late_spring = (cgrid%lat(ipy) >= 0.0 .and. month == 6 .or. month == 7) .or.                          &
      ...
      siteloop: do isi = 1,cpoly%nsites
         ...
         patchloop: do ipa = 1,csite%npatches
            ...
            organic_layer_depth = (csite%fast_soil_C(ipa) + csite%slow_soil_C(ipa))        &
            ...
            do ico = 1, cpatch%ncohorts
               ...
               !at crit_olayer_max
               thispft    = cpatch%pft(ico)
               mort_fac = 1.0
               select case(thispft)
               case (21)
                  if (organic_layer_depth >= crit_olayer_min) then
                     mort_fac = (seedling_mortality(thispft) +							  &
                          (1 - seedling_mortality(thispft))                               &
                          * aspen_seedling_mort                                           &
                          *(min(organic_layer_depth,crit_olayer_max) - crit_olayer_min)/  &
                          (crit_olayer_max - crit_olayer_min)) &
                          /seedling_mortality(thispft)
                  end if
 
               case (22)
                  if (organic_layer_depth >= crit_olayer_min) then
                     mort_fac = (seedling_mortality(thispft) +                             &
                          (1 - seedling_mortality(thispft))*blackspruce_seedling_mort      &
                          *(min(organic_layer_depth,crit_olayer_max) - crit_olayer_min)/   &
                          (crit_olayer_max - crit_olayer_min))                             &
                          /seedling_mortality(thispft)
                  end if
...
!               print*,'mortfac',mort_fac,'total mort',mort_fac*seedling_mortality(cpatch%pft(ico))

               nbefore = nbefore + cpatch%nplant(ico) * cpatch%bseeds(ico) * csite%area(ipa)    &
                    * (1.0-mort_fac * seedling_mortality(cpatch%pft(ico)))                      &
                    /c2n_recruit(cpatch%pft(ico))
               pn1 = pn1 + csite%area(ipa) * cpatch%nplant(ico) * (cpatch%balive(ico)           &
                    / c2n_leaf(cpatch%pft(ico)) + cpatch%bdead(ico) /                           &
                    c2n_stem(cpatch%pft(ico))  + cpatch%nstorage(ico) + cpatch%bseeds(ico) *    &
                    (1.0  - mort_fac * seedling_mortality(cpatch%pft(ico)))                     &
                    / c2n_recruit(cpatch%pft(ico)))
            enddo
            ...
            pftloop: do ipft = 1, n_pft
               ...
               if( include_pft(ipft)                                         .and.         &
                  ...
                  if(csite%dist_type(ipa) /= 1 .or. include_pft_ag(ipft)) then
                     ...
                     rectest%balive    = rectest%bleaf * (1 + q(ipft)) * 2 &  ! take consideration of bstorage
										+ dbh2bs(rectest%dbh,ipft)
...
!                     if (.false.)then
            ...
            if (ncohorts_new > cpatch%ncohorts) then
               ...
               recloop: do ico = temppatch%ncohorts+1,ncohorts_new
                  ...
                  !------------------------------------------------------------------------!
                  call pheninit_balive_bstorage(nzg,csite,ipa,ico,csite%ntext_soil(:,ipa)  &!ATT
                                               ,full_flush)
                  !------------------------------------------------------------------------!
         ...
         update_patch_loop: do ipa = 1,csite%npatches
            ...
            if(cpatch%ncohorts > 0 .and. maxcohort >= 0) then
               ...
               call fuse_cohorts(csite,ipa, csite%green_leaf_factor(:,ipa),cpoly%lsl(isi))                         
               call split_cohorts(cpatch, csite%green_leaf_factor(:,ipa),cpoly%lsl(isi))
            end if
            ...
            do ico = 1,cpatch%ncohorts
               pn2 = pn2 + csite%area(ipa) * cpatch%nplant(ico) * (cpatch%balive(ico) /     &
                     c2n_leaf(cpatch%pft(ico)) + cpatch%bdead(ico) /                        &
                     c2n_stem(cpatch%pft(ico)) + cpatch%nstorage(ico))
            enddo
...
!   print *,'organic_layer_depth', organic_layer_depth,'npatches',csite%npatches
  ! print*,'fast_soil_C', csite%fast_soil_C,'slowfast_soil_C', csite%slow_soil_C!ATT
   ...
   case (1)
      ...
      siteloop1: do isi = 1,cpoly%nsites
         ...
         donpaloop1: do donpa = 1,csite%npatches
            ...
            !ATT calculate organic layer depth
            organic_layer_depth = (csite%fast_soil_C(donpa) + csite%slow_soil_C(donpa))    &
                                / organic_matter_density
            ...
            doncoloop1: do donco = 1, donpatch%ncohorts
               ...
               case (21)
                  if (organic_layer_depth >= crit_olayer_min) then
                     mort_fac = (seedling_mortality(donpft) +							  &
                          (1 - seedling_mortality(donpft))                                &
                          * aspen_seedling_mort                                           &
                          *(min(organic_layer_depth,crit_olayer_max) - crit_olayer_min)/  &
                          (crit_olayer_max - crit_olayer_min)) &
                          /seedling_mortality(donpft)
                  end if
 
               case (22)
                  if (organic_layer_depth >= crit_olayer_min) then
                     mort_fac = (seedling_mortality(donpft) +                             &
                          (1 - seedling_mortality(donpft))*blackspruce_seedling_mort      &
                          *(min(organic_layer_depth,crit_olayer_max) - crit_olayer_min)/  &
                          (crit_olayer_max - crit_olayer_min))                            &
                          /seedling_mortality(donpft)
                  end if
               ...
               if (phenology(donpft) /= 2 .or. late_spring) then
                  nseedling   = donpatch%nplant(donco) * donpatch%bseeds(donco)            &
                              * (1.0 - mort_fac * seedling_mortality(donpft))
