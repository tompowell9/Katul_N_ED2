...
module disturbance_utils
      ...
      polyloop: do ipy = 1,cgrid%npolygons
         ...
         siteloop: do isi = 1,cpoly%nsites
            ...
            do ipa = 1, csite%npatches
               ...
               soilN1 = soilN1 + (csite%fast_soil_N(ipa) + csite%slow_soil_N(ipa) +       &!ATT
                        csite%structural_soil_C(ipa)/c2n_structural +                     &
                        csite%mineralized_soil_N(ipa)) * csite%area(ipa)    

               do ico = 1, cpatch%ncohorts
                  plantN1 = plantN1 + csite%area(ipa) * cpatch%nplant(ico) *               &
            ...
            print*,'fast_soil_N',csite%fast_soil_N,'slow_soil_N ',csite%slow_soil_N,'area', csite%area
            ...
            new_lu_loop: do new_lu = 1, n_dist_types
               ...
               if (area > min_new_patch_area) then
                ...
                !----- If the new patch is agriculture, plant it with grasses. ----------!
                   !do not use input ntext soil here ATT
                  ...
                  if (new_lu == 1) then 
                     !do not use input ntext soil here ATT
                      call plant_patch(csite,new_lu+onsp,nzg,cpoly%agri_stocking_pft(isi)   &
                                     ...
                                     ,csite%ntext_soil(:,new_lu+onsp)                              &
                                     ,csite%green_leaf_factor(:,new_lu+onsp), 1.0, cpoly%lsl(isi))
                     end if
                  qpatch => csite%patch(new_lu+onsp)

                  !----- Fuse then terminate cohorts. -------------------------------------!
                  if (csite%patch(new_lu+onsp)%ncohorts > 0 .and. maxcohort >= 0) then
                     call fuse_cohorts(csite,new_lu+onsp,csite%green_leaf_factor(:,new_lu+onsp)    &
                                      ,cpoly%lsl(isi))
                     call terminate_cohorts(csite,new_lu+onsp,elim_nplant,elim_lai)
                     call split_cohorts(qpatch,csite%green_leaf_factor(:,new_lu+onsp)              &
                                       ,cpoly%lsl(isi))
                  end if
            ...
            do ipa = 1, csite%npatches
               ...
               soilN2 = soilN2 + (csite%fast_soil_N(ipa) + csite%slow_soil_N(ipa) +         &!ATT
                        csite%structural_soil_C(ipa)/c2n_structural +                       &
                        csite%mineralized_soil_N(ipa)) * csite%area(ipa)
                    
               do ico = 1, cpatch%ncohorts
                  plantN2 = plantN2 + csite%area(ipa) * cpatch%nplant(ico) *                &
                           (cpatch%balive(ico)/c2n_leaf(cpatch%pft(ico)) +                  &
                           cpatch%bdead(ico)/c2n_structural  + cpatch%nstorage(ico)) !JL!
               enddo
            enddo
   ...
   subroutine initialize_disturbed_patch(csite,atm_tmp,np,dp,lsl)
      ...
      csite%rough                      (np) = 0.0
      csite%fsc_in                     (np) = 0.0
      csite%slsc_in                    (np) = 0.0
      csite%slsn_in                    (np) = 0.0
      csite%ssc_in                     (np) = 0.0
      csite%ssl_in                     (np) = 0.0
      csite%fsn_in                     (np) = 0.0
   ...
   subroutine increment_patch_vars(csite,np, cp, area_fac)
      ...
      csite%fast_soil_N                (np) = csite%fast_soil_N                (np)        &
                                            ...
                                            * area_fac
      csite%slow_soil_N                (np) = csite%slow_soil_N                (np)        &!ATT
                                            + csite%slow_soil_N                (cp)        & 
                                            * area_fac
      ...
      csite%fsc_in                     (np) = csite%fsc_in                     (np)        &
                                            + csite%fsc_in                     (cp)        &
                                            * area_fac
      csite%slsc_in                    (np) = csite%slsc_in                    (np)        &
                                            + csite%slsc_in                    (cp)        &
                                            * area_fac
      csite%slsn_in                    (np) = csite%slsn_in                    (np)        &
                                            + csite%slsn_in                    (cp)        &
                                            * area_fac
      ...
      do k = 1, nzg
         ...
         csite%soil_water(k,np)             = csite%soil_water               (k,np)        &
                                            ...
                                            * area_fac
         csite%ntext_soil(k,np) = csite%ntext_soil(k,cp)
      end do
      print*,'np',np,'csite%ntext_soil',csite%ntext_soil(:,:)
      return
   ...
   subroutine accum_dist_litt(csite,np,cp,q,area_fac,loss_fraction,poly_dest_type          &
      ...
      integer                                       :: ipft
      real                                          :: fast_litter
      real                                          :: slow_litter!ATT
      real                                          :: slow_litter_n!ATT
      real                                          :: struct_litter
      real                                          :: struct_lignin
      real                                          :: fast_litter_n
      real                                          :: struct_cohort
      ...
      do ico = 1,cpatch%ncohorts
         ipft = cpatch%pft(ico)
         slow_litter   = slow_litter                                                       &
                       + (1. - survivorship(q,poly_dest_type,mindbh_harvest,csite,cp,ico)) &
                       * (f_labile(ipft) * cpatch%balive(ico) + cpatch%bstorage(ico))      &
                       * cpatch%nplant(ico) * (1 - f_fast(ipft))
         slow_litter_n = slow_litter_n                                                     &
                       + (1. - survivorship(q,poly_dest_type,mindbh_harvest,csite,cp,ico)) &
                       * (f_labile(ipft) * cpatch%balive(ico) / c2n_leaf(ipft)             &
                         + cpatch%nstorage(ico)) * cpatch%nplant(ico) * (1 - f_fast(ipft))
 
         fast_litter   = fast_litter                                                       &
                       + (1. - survivorship(q,poly_dest_type,mindbh_harvest,csite,cp,ico)) &
                       * ( f_labile(ipft) * cpatch%balive(ico) + cpatch%bstorage(ico))     &
                       * cpatch%nplant(ico) * f_fast(ipft)
         fast_litter_n = fast_litter_n                                                     &
                       + (1. - survivorship(q,poly_dest_type,mindbh_harvest,csite,cp,ico)) &
                       * ( f_labile(ipft) * cpatch%balive(ico) / c2n_leaf(ipft)            &
                         + cpatch%nstorage(ico)) * cpatch%nplant(ico) * f_fast(ipft)   
      ...
      csite%slow_soil_C(np)       = csite%slow_soil_C(np)       + slow_litter   * area_fac
      ...
      csite%slow_soil_N(np)       = csite%slow_soil_N(np)       + slow_litter_n * area_fac!ATT
      return
   ...
   subroutine plant_patch(csite,np,mzg,pft,density,ntext_soil,green_leaf_factor            &
      ...
      !      phenology (or start with 1.0 if the plant doesn't                             !
      !      shed their leaves due to water stress.                                        !
      !------------------------------------------------------------------------------------!
      call pheninit_balive_bstorage(mzg,csite,np,nc,ntext_soil,green_leaf_factor)
      !dont need to put in a patch loop because ntext soil is not actually used in this routine
      !------------------------------------------------------------------------------------!
