...
subroutine apply_forestry(cpoly, isi, year)
   ...
   if (total_harvested_area > min_new_patch_area) then
      ...
      call update_patch_thermo_props(csite,newp,newp,nzg,nzs)      
      ...
      if (cpoly%plantation(isi) == 1 .and. year > plantation_year) then
            call plant_patch(csite,newp,nzg,cpoly%plantation_stocking_pft(isi)                &
                         ...
                         ,csite%ntext_soil(:,newp),csite%green_leaf_factor(:,newp),2.0       &   !ATT, no need to modify
                         ,cpoly%lsl(isi))
            csite%plantation(newp) = 1
      end if
      call update_patch_derived_props(csite,cpoly%lsl(isi),cpoly%met(isi)%prss,newp)
      call new_patch_sfc_props(csite,newp,nzg,nzs,cpoly%ntext_soil(:,isi))                !ATT, no nee to modify
...
subroutine norm_harv_patch(csite,newp)
   ...
   csite%slow_soil_N(newp)                 = csite%slow_soil_N(newp)         * area_fac
   ...
   csite%slsc_in(newp)                     = csite%slsc_in(newp)             * area_fac
   csite%slsn_in(newp)                     = csite%slsn_in(newp)             * area_fac
