...
subroutine update_C_and_N_pools(cgrid)
   ...
   use  soil_coms        , only : soil                            & !the structure stores the soil information  !JL
                           ...
                           , slz                                  &
						   , nlsl								  ! soil depth of each layer                  !JL
  use grid_coms          , only: nzg
  use consts_coms        , only : day_sec			  &
  			        , wdns
  use nutrient_constants , only: leaching_efficiency_factor       &  
                               , gas_loss_rate                    &
							   , N_deposition
   ...
   polygonloop: do ipy = 1,cgrid%npolygons
      ...
      siteloop: do isi = 1,cpoly%nsites
         ...
         patchloop: do ipa = 1,csite%npatches
            ...
            slow_N_loss  = csite%today_A_decomp(ipa) * K3 * csite%slow_soil_N(ipa)!ATT
            ...
            !------------------------------------------------------------------------------!
            !----INITIALIZATION------------------------------------------------------------!
             total_soil_water = sum(csite%soil_water(nlsl:nzg,ipa) * dslz(nlsl:nzg)) * wdns
            ...
            ! kg N/m2/day
            csite%N_leached(ipa) = max(MSN_after_plant_uptake / total_soil_water           & ! kgN/kgH2O
                                   * csite%dmean_nutrient_layer_drainage(ipa)              & ! kgH2O/m2/sec
                                   * leaching_efficiency_factor * day_sec,0.)				 ! sec/day
!			print*,'nl drainage',csite%dmean_nutrient_layer_drainage(ipa),'N_leached',csite%N_leached(ipa),&
!					'MSN',MSN_after_plant_uptake,'total_soil_water',total_soil_water,&
!					'leaching_efficiency_factor',leaching_efficiency_factor,'day_sec',day_sec
!			print*,'soil_water',csite%soil_water(nlsl:nzg,ipa)
!			print*,'N_depo',N_deposition
            ...
            !----- Mineralized pool. ------------------------------------------------------!
            csite%mineralized_N_input(ipa) = fast_N_loss + slow_N_loss                     &!ATT
                                            +  structural_N_loss                           & 
                                            +  N_deposition 
            ...
            csite%total_plant_nitrogen_uptake(ipa) = 0.0   !XXT ATT
            !------------------------------------------------------------------------------!
            ...
            csite%slow_soil_C(ipa)       = csite%slow_soil_C(ipa) + csite%slsc_in(ipa)     &!ATT
										   - slow_C_loss
            !------------------------------------------------------------------------------!
            ...
            csite%slow_soil_N(ipa)        = csite%slow_soil_N(ipa) + csite%slsn_in(ipa)    &!ATT
           								  - slow_N_loss
            !------------------------------------------------------------------------------!
            !      Force all pools to be either zero or positive.                          !
            !------------------------------------------------------------------------------!
            if (csite%fast_soil_C(ipa) < 0)then
               print*,'NEGATIVE!! fast_soil_C',csite%fast_soil_C(ipa)
            endif
            if (csite%structural_soil_C(ipa) < 0)then
               print*,'NEGATIVE!! structural_soil_C',csite%structural_soil_C(ipa)
            endif
            if (csite%slow_soil_C(ipa) < 0)then
               print*,'NEGATIVE!! slow_soil_C',csite%slow_soil_C(ipa)
            endif
            if (csite%fast_soil_N(ipa) < 0)then
               print*,'NEGATIVE!! fast_soil_N',csite%fast_soil_N(ipa)
            endif
            if (csite%mineralized_soil_N(ipa) < 0)then
               print*,'NEGATIVE!! mineralized_soil_N',csite%mineralized_soil_N(ipa)
            endif
            if (csite%slow_soil_N(ipa) < 0)then
               print*,'NEGATIVE!! slow_soil_N',csite%slow_soil_N(ipa)
            endif
            ...
            csite%slow_soil_N(ipa)        = max(0.0,csite%slow_soil_N(ipa))!ATT
if(isnan(csite%fast_soil_N(ipa)+csite%mineralized_soil_N(ipa)+csite%slow_soil_N(ipa))) then
	print*,'FSN',csite%fast_soil_N(ipa),'MSN',csite%mineralized_soil_N(ipa),'SSN',&
			csite%slow_soil_N(ipa)
endif
