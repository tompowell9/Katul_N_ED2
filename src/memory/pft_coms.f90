...
module pft_coms
   ...
   ! when h2o_plant_lim == 3 ,water_conductancce represents the conductivity of
   ! stems, unit is kgH2O m-1 s-1 MPa-1

   !---  variables used when h2o_plant_lim == 3
   real, dimension(n_pft) :: psi50
   !----- water potential at which 50% of conductivity is lost   [ mH2O]  ------------!

   real, dimension(n_pft) :: leaf_psi50    
   !----- leaf water potential at which 50% of Vm0 is lost       [ mH2O]  ------------!

   real, dimension(n_pft) :: TLP          
   !----- turgor loss point		[mH2O]

   real, dimension(n_pft) :: root_beta
   !----  root_beta,   parameter used to describe how quickly the root biomass

   real, dimension(n_pft) :: C_resorption_factor
   !---- Carbon resorbed from fine roots and leaves when they turnover

   real, dimension(n_pft) :: N_resorption_factor 
   !---- Nitrogen resorbed from fine roots and leaves when they turnover
   
   
   real, dimension(n_pft) :: Cap_stem
   !---- stem water capcacity kgH2O/m3/MPa
   real, dimension(n_pft) :: Cap_leaf
   !---- stem water capcacity kgH2O/m2/MPa
   !=======================================================================================!
   !=======================================================================================!
   ...
   real   , dimension(n_pft)    :: SRA  ! specific root area m2/kgC
   real   , dimension(n_pft)    :: root_dens  ! density of root tissue kgC/m3
   ...
   !----- DBH-sapwood allometry intercept for large DBH cohorts. -----------------------------!
   real   , dimension(n_pft)    :: b1Bsap
   !----- DBH-sapwood allometry slope for large DBH cohorts. ---------------------------------!
   real   , dimension(n_pft)    :: b2Bsap
