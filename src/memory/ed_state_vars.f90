...
!============================================================================!
  ...
  type patchtype
     ...
     real ,pointer,dimension(:) :: bstorage_min
      !Plant storage pool of nitroge [kgN/plant]
     real ,pointer,dimension(:) :: nstorage
     real ,pointer,dimension(:) :: nstorage_min
     ...
     real, pointer, dimension(:)   :: today_nppdaily  !kgC /m2 /s
 ...
 real ,pointer,dimension(:) :: nitrogen_uptake !JL!
     ...
     ! Water supply from the soil in fast time scale (kg_H2O)
     real , pointer, dimension(:) :: water_supply
     real, pointer, dimension(:,:) :: water_supply_layer_frac  !contribution to water_supply of different soil layers

     real, pointer, dimension(:)   :: psi_leaf  ! records the leaf water potential unit is mH2O
     real, pointer, dimension(:)   :: psi_stem  ! records the stem water potential unit is mH2O
	 real, pointer, dimension(:)   :: rl_flow   ! records the root-to-leaf water flow (sapflow) kg H2O/s
	 real, pointer, dimension(:)   :: sr_flow   ! records the soil-to-root water flow kg H2O/s
	 ...
	 !XXT, leaf water potential
	 real , pointer, dimension(:)   :: predawn_psi_leaf
	 real , pointer, dimension(:)   :: midday_psi_leaf
	 real , pointer, dimension(:)   :: predawn_psi_stem
	 real , pointer, dimension(:)   :: midday_psi_stem

	 !fast analysis output
	 real , pointer, dimension(:)   :: avg_psi_stem
	 real , pointer, dimension(:)   :: avg_psi_leaf
	 real , pointer, dimension(:)   :: avg_rl_flow
	 real , pointer, dimension(:)   :: avg_sr_flow
	 real , pointer, dimension(:)   :: avg_m_coef
     ...
     real, pointer, dimension(:) :: m_coef  ! leaf water stress factor used to control Vm0 etc.
     real, pointer, dimension(:) :: hite_coef  ! XXT, used to adjust Vm0 according to hite
     real, pointer, dimension(:,:) :: past_psi_leaf  ! XXT, used to adjust Vm0 according to hite

     ! cohort_id to track cohorts ATT
     integer ,pointer,dimension(:) :: cohort_id

	 !by XXT, record the stomatal conductance last time
	 !in order to speed up the integration in katul_stomatal.f90
	 
     real ,pointer,dimension(:) :: former_gV   ! Rubisco limited
	 real ,pointer,dimension(:) :: former_gJ   ! Light limited
  end type patchtype
!============================================================================!
!============================================================================!
...
!============================================================================!
  ...
  type sitetype
     ...
     ! Soil nitrogen concentration, slow pool (kg/m2)
     real , pointer,dimension(:) :: slow_soil_N 

     ! Number of degree days
     ! Degree days --- sum of daily average temperatures above 278.15 K 
     real , pointer,dimension(:) :: sum_dgd, sum_cdd
     ...
     integer, pointer, dimension(:,:) :: phenophase
     real, pointer, dimension(:,:) :: green_leaf_factor, leaf_aging_factor
     ...
     integer,pointer,dimension(:,:) :: ntext_soil!ATT
     ...
     ! Input to slow soil carbon pool [kgC/m2/day]   ATT
     real , pointer,dimension(:) :: slsc_in
     ...
     ! Input to slower decomposing soil nitrogen pool [kgN/m2/day]
     real , pointer,dimension(:) :: slsn_in !ATT
     ...
     !----- Hydrology
     real, pointer, dimension(:)   :: dmean_nutrient_layer_drainage ! drainage out of nutrient layer XXT [k/m2/s]
     ...
     real,pointer,dimension(:)   :: avg_drainage    ! Average water drainage through the lower 
                                                    !  soil layer                              [kg/m2/s]
     real,pointer,dimension(:)   :: avg_nutrient_layer_drainage    ! Average water drainage through the lower 
                                                    !  soil layer                              [kg/m2/s]
     ...
     real,pointer,dimension(:,:,:) :: past_psi  ! k,day,patch   past 10 day soil moisture
...
!     real,pointer,dimension(:,:) :: leaf_aging_factor !(n_pft,nsites)
     ...
     real(kind=8), pointer,dimension(:)  :: PlantN           
     real(kind=8), pointer,dimension(:)  :: ForestN         
     real(kind=8), pointer,dimension(:)  :: SoilN          
...
!============================================================================!
  subroutine allocate_sitetype(csite,npatches)
    ...
    allocate(csite%slow_soil_N(npatches))!ATT!
    allocate(csite%sum_dgd(npatches))
    allocate(csite%sum_chd(npatches))
    allocate(csite%sum_cdd(npatches))
    allocate(csite%N_leached(npatches))!JL!
    allocate(csite%N_gas_loss(npatches))!JL!
    ...
    allocate(csite%green_leaf_factor(n_pft,npatches))
    allocate(csite%leaf_aging_factor(n_pft,npatches))
    allocate(csite%phenophase(n_pft,npatches))
    ...
    allocate(csite%past_psi(nzg,31,npatches))
...
!============================================================================!
  subroutine allocate_patchtype(cpatch,ncohorts)
    ...
    allocate(cpatch%bstorage_min(ncohorts))
    allocate(cpatch%nstorage(ncohorts))
    allocate(cpatch%nstorage_min(ncohorts))
    ...
    allocate(cpatch%midday_psi_stem(ncohorts))
    allocate(cpatch%predawn_psi_stem(ncohorts))
    allocate(cpatch%midday_psi_leaf(ncohorts))
    allocate(cpatch%predawn_psi_leaf(ncohorts))

    allocate(cpatch%avg_psi_leaf(ncohorts))
    allocate(cpatch%avg_m_coef(ncohorts))
    allocate(cpatch%avg_psi_stem(ncohorts))
    allocate(cpatch%avg_rl_flow(ncohorts))
    allocate(cpatch%avg_sr_flow(ncohorts))

    allocate(cpatch%rl_flow(ncohorts))
    allocate(cpatch%sr_flow(ncohorts))
    allocate(cpatch%hite_coef(ncohorts))
    allocate(cpatch%m_coef(ncohorts))
    allocate(cpatch%former_gJ(ncohorts))
    allocate(cpatch%former_gV(ncohorts))
    allocate(cpatch%water_supply_layer_frac(nzg,ncohorts)) !XXT
    ...
    !XXT for the phenology of tropical deciduous forest
    allocate(cpatch%past_psi_leaf(31,ncohorts))
...
!============================================================================!
  subroutine nullify_sitetype(csite)
    ...
    nullify(csite%phenophase)
    nullify(csite%green_leaf_factor)
    nullify(csite%leaf_aging_factor)
...
!============================================================================!
  subroutine nullify_patchtype(cpatch)
    ...
    nullify(cpatch%bstorage_min)
    nullify(cpatch%nstorage)
    nullify(cpatch%nstorage_min)
    nullify(cpatch%cb)
...
nullify(cpatch%carbon_balance_pot)!JL!
    ...
    nullify(cpatch%predawn_psi_leaf)
    nullify(cpatch%midday_psi_leaf)
    nullify(cpatch%predawn_psi_stem)
    nullify(cpatch%midday_psi_stem)


    nullify(cpatch%avg_psi_leaf)
    nullify(cpatch%avg_psi_stem)
    nullify(cpatch%avg_m_coef)
    nullify(cpatch%avg_rl_flow)
    nullify(cpatch%avg_sr_flow)

    nullify(cpatch%rl_flow)
    nullify(cpatch%sr_flow)
    nullify(cpatch%hite_coef)
    nullify(cpatch%m_coef)
    nullify(cpatch%former_gJ)
    nullify(cpatch%former_gV)
    nullify(cpatch%water_supply_layer_frac)
...
!============================================================================!
  subroutine deallocate_sitetype(csite)
    ...
    if(associated(csite%slow_soil_N                  )) deallocate(csite%slow_soil_N                  )!ATT
    ...
    if(associated(csite%sum_cdd                      )) deallocate(csite%sum_cdd                      )
    if(associated(csite%N_leached           )) deallocate(csite%N_leached           )!JL!
    if(associated(csite%N_gas_loss        )) deallocate(csite%N_gas_loss          )!JL!
    ...
    if(associated(csite%green_leaf_factor            )) deallocate(csite%green_leaf_factor            )
    if(associated(csite%leaf_aging_factor            )) deallocate(csite%leaf_aging_factor            )
    if(associated(csite%phenophase                   )) deallocate(csite%phenophase                   )
    if(associated(csite%soil_water                   )) deallocate(csite%soil_water                   )
    if(associated(csite%ntext_soil                   )) deallocate(csite%ntext_soil                   )!ATT
    ...
    if(associated(csite%slsc_in                      )) deallocate(csite%slsc_in                      )
    ...
    if(associated(csite%slsn_in                      )) deallocate(csite%slsn_in                      )!ATT
    ...
    if(associated(csite%past_psi))deallocate(csite%past_psi)
...
!============================================================================!
  subroutine deallocate_patchtype(cpatch)
    ...
    if(associated(cpatch%bstorage_min))        deallocate(cpatch%bstorage_min)
    if(associated(cpatch%nstorage))            deallocate(cpatch%nstorage)
    if(associated(cpatch%nstorage_min))		   deallocate(cpatch%nstorage_min)
    ...
    if(associated(cpatch%psi_leaf))           deallocate(cpatch%psi_leaf)
    if(associated(cpatch%psi_stem))           deallocate(cpatch%psi_stem)
    if(associated(cpatch%predawn_psi_leaf))          deallocate(cpatch%predawn_psi_leaf)
    if(associated(cpatch%midday_psi_leaf))          deallocate(cpatch%midday_psi_leaf)
    if(associated(cpatch%predawn_psi_stem))          deallocate(cpatch%predawn_psi_stem)
    if(associated(cpatch%midday_psi_stem))          deallocate(cpatch%midday_psi_stem)


    if(associated(cpatch%avg_psi_leaf))           deallocate(cpatch%avg_psi_leaf)
    if(associated(cpatch%avg_psi_stem))           deallocate(cpatch%avg_psi_stem)
    if(associated(cpatch%avg_m_coef))           deallocate(cpatch%avg_m_coef)
    if(associated(cpatch%avg_rl_flow))           deallocate(cpatch%avg_rl_flow)
    if(associated(cpatch%avg_sr_flow))           deallocate(cpatch%avg_sr_flow)

    if(associated(cpatch%rl_flow))           deallocate(cpatch%rl_flow)
    if(associated(cpatch%sr_flow))           deallocate(cpatch%sr_flow)
    if(associated(cpatch%hite_coef))           deallocate(cpatch%hite_coef)
    if(associated(cpatch%m_coef))           deallocate(cpatch%m_coef)
    if(associated(cpatch%former_gJ))           deallocate(cpatch%former_gJ)
    if(associated(cpatch%former_gV))           deallocate(cpatch%former_gV)
    if(associated(cpatch%water_supply_layer_frac))deallocate(cpatch%water_supply_layer_frac)
    ...
    ! XXT for the phenology of tropical deciduous forest
    if(associated(cpatch%past_psi_leaf))      deallocate(cpatch%past_psi_leaf)
...
!============================================================================!
   ...
   subroutine copy_sitetype(isite,osite,ipaa,ipaz,opaa,opaz)
      ...
      opaloop: do opa = opaa, opaz
         ...
         osite%slow_soil_N(opa)                 = isite%slow_soil_N(ipa)                     !ATT
         osite%sum_dgd(opa)                     = isite%sum_dgd(ipa)
         osite%sum_chd(opa)                     = isite%sum_chd(ipa)
         osite%sum_cdd(opa)                     = isite%sum_cdd(ipa)
         ...
         do k = 1, n_pft
            osite%green_leaf_factor(k,opa) = isite%green_leaf_factor(k,ipa)
            osite%leaf_aging_factor(k,opa) = isite%leaf_aging_factor(k,ipa)
            osite%phenophase(k,opa) = isite%phenophase(k,ipa)
         enddo
         do k=1,nzg
            osite%soil_energy(k,opa)            =  isite%soil_energy(k,ipa)
            osite%soil_water(k,opa)             =  isite%soil_water(k,ipa)
            osite%ntext_soil(k,opa)             =  isite%ntext_soil(k,ipa)!ATT
            ...
            enddo
            do ihgt = 1, 31
               osite%past_psi(k,ihgt,opa) = isite%past_psi(k,ihgt,ipa)
            enddo
         end do
         ...
         !----- Daily averages. -----------------------------------------------------------!
            osite%dmean_nutrient_layer_drainage (opa) = & 
				isite%dmean_nutrient_layer_drainage (ipa)
         if (idoutput > 0 .or. imoutput > 0 .or. iqoutput > 0) then
...
!============================================================================!
  subroutine copy_sitetype_mask(sitein,siteout,logmask,masksz,newsz)
    ...
    siteout%slow_soil_N(1:inc)          = pack(sitein%slow_soil_N,logmask)               !ATT
    siteout%sum_dgd(1:inc)              = pack(sitein%sum_dgd,logmask)
    siteout%sum_chd(1:inc)              = pack(sitein%sum_chd,logmask)
    siteout%sum_cdd(1:inc)              = pack(sitein%sum_cdd,logmask)
    ...
    siteout%slsc_in(1:inc)              = pack(sitein%slsc_in,logmask)
    ...
    siteout%slsn_in(1:inc)              = pack(sitein%slsn_in,logmask)
    ...
    siteout%avg_nutrient_layer_drainage(1:inc)         = pack(sitein%avg_nutrient_layer_drainage,logmask)
    ...
    ! Soil layers 1:nzg
    do k = 1, n_pft
       siteout%green_leaf_factor(k,1:inc) = pack(sitein%green_leaf_factor(k,:),logmask)
       siteout%leaf_aging_factor(k,1:inc) = pack(sitein%leaf_aging_factor(k,:),logmask)
       siteout%phenophase(k,1:inc)        = pack(sitein%phenophase(k,:),logmask)
    enddo

    do k=1,nzg
       siteout%soil_energy(k,1:inc)        = pack(sitein%soil_energy(k,:),logmask)
       siteout%soil_water(k,1:inc)         = pack(sitein%soil_water(k,:),logmask)
       siteout%ntext_soil(k,1:inc)         = pack(sitein%ntext_soil(k,:),logmask)!ATT
       ...
       do m = 1, 31
          siteout%past_psi(k,m,1:inc) = pack(sitein%past_psi(k,m,:),logmask)
       enddo
    end do
    ...
    do m=1,newsz
       ...
       siteout%dmean_nutrient_layer_drainage (1:inc) = &
			pack(sitein%dmean_nutrient_layer_drainage ,logmask)
...
!============================================================================!
  subroutine copy_patchtype_mask(patchin,patchout,mask,masksz,newsz)
    ...
    patchout%bstorage_min(1:inc)     = pack(patchin%bstorage_min,mask)
    patchout%nstorage(1:inc)         = pack(patchin%nstorage,mask)
    patchout%nstorage_min(1:inc)     = pack(patchin%nstorage_min,mask)
    ...
    patchout%cohort_id(1:inc)        = pack(patchin%cohort_id,mask)  

    patchout%psi_leaf(1:inc)         = pack(patchin%psi_leaf,mask)
    patchout%psi_stem(1:inc)         = pack(patchin%psi_stem,mask)
    patchout%midday_psi_leaf(1:inc)         = pack(patchin%midday_psi_leaf,mask)
    patchout%midday_psi_stem(1:inc)         = pack(patchin%midday_psi_stem,mask)
    patchout%predawn_psi_leaf(1:inc)         = pack(patchin%predawn_psi_leaf,mask)
    patchout%predawn_psi_stem(1:inc)         = pack(patchin%predawn_psi_stem,mask)

    patchout%rl_flow(1:inc)         = pack(patchin%rl_flow,mask)
    patchout%sr_flow(1:inc)         = pack(patchin%sr_flow,mask)
    patchout%hite_coef(1:inc)         = pack(patchin%hite_coef,mask)
    patchout%m_coef(1:inc)         = pack(patchin%m_coef,mask)
    patchout%former_gJ(1:inc)         = pack(patchin%former_gJ,mask)
    patchout%former_gV(1:inc)         = pack(patchin%former_gV,mask)


    patchout%avg_psi_leaf(1:inc)         = pack(patchin%avg_psi_leaf,mask)
    patchout%avg_psi_stem(1:inc)         = pack(patchin%avg_psi_stem,mask)
    patchout%avg_m_coef(1:inc)         = pack(patchin%avg_m_coef,mask)
    patchout%avg_rl_flow(1:inc)         = pack(patchin%avg_rl_flow,mask)
    patchout%avg_sr_flow(1:inc)         = pack(patchin%avg_sr_flow,mask)
    do m=1,inc
       k=incmask(m)
       do i = 1,13
       ...
       !XXT for the phenology of tropical deciduous forest
       do i = 1,31
          patchout%past_psi_leaf(i,m)       = patchin%past_psi_leaf(i,k)
       end do
       !XXT water layer frac
       do i = 1,nzg
          patchout%water_supply_layer_frac(i,m)  = patchin%water_supply_layer_frac(i,k)
       enddo
    end do
...
!============================================================================!
  subroutine copy_patchtype(patchin,patchout,ipin1,ipin2,ipout1,ipout2)
    ...
    do iout=ipout1,ipout2
       ...
       patchout%bstorage_min(iout)     = patchin%bstorage_min(iin)
       patchout%nstorage(iout)         = patchin%nstorage(iin)
       patchout%nstorage_min(iout)     = patchin%nstorage_min(iin)
       ...
       patchout%psi_leaf(iout)         = patchin%psi_leaf(iin)
       patchout%psi_stem(iout)         = patchin%psi_stem(iin)
       patchout%predawn_psi_leaf(iout)         = patchin%predawn_psi_leaf(iin)
       patchout%predawn_psi_stem(iout)         = patchin%predawn_psi_stem(iin)
       patchout%midday_psi_leaf(iout)         = patchin%midday_psi_leaf(iin)
       patchout%midday_psi_stem(iout)         = patchin%midday_psi_stem(iin)

       patchout%rl_flow(iout)         = patchin%rl_flow(iin)
       patchout%sr_flow(iout)         = patchin%sr_flow(iin)
       patchout%hite_coef(iout)         = patchin%hite_coef(iin)
       patchout%m_coef(iout)         = patchin%m_coef(iin)
       patchout%former_gV(iout)         = patchin%former_gV(iin)
       patchout%former_gJ(iout)         = patchin%former_gJ(iin)


       patchout%avg_psi_leaf(iout)         = patchin%avg_psi_leaf(iin)
       patchout%avg_psi_stem(iout)         = patchin%avg_psi_stem(iin)
       patchout%avg_m_coef(iout)         = patchin%avg_m_coef(iin)
       patchout%avg_rl_flow(iout)         = patchin%avg_rl_flow(iin)
       patchout%avg_sr_flow(iout)         = patchin%avg_sr_flow(iin)

       patchout%cohort_id(iout)        = patchin%cohort_id(iin)
       patchout%past_psi_leaf(:,iout)  = patchin%past_psi_leaf(:,iin)

       patchout%water_supply_layer_frac(:,iout)   = patchin%water_supply_layer_frac(:,iin)
   ...
   subroutine filltab_polygontype(igr,ipy,init)
      ...
      if (associated(cpoly%ntext_soil)) then
         nvar=nvar+1
           ...
           var_len,var_len_global,max_ptrs,'NTEXT_SOIL_SI :220:hist:mont:year') 
         call metadata_edio(nvar,igr,'No metadata available','[NA]','NA') 
      end if
     ...
     ! end if
      ...
      if (associated(cpoly%basal_area_cut)) then
         nvar=nvar+1
           ...
           var_len,var_len_global,max_ptrs,'BASAL_AREA_CUT :246:year:mont:hist') 
         call metadata_edio(nvar,igr,'No metadata available','[NA]','NA') 
      end if
      ...
      if (associated(cpoly%agb_mort)) then
         nvar=nvar+1
           ...
           var_len,var_len_global,max_ptrs,'AGB_MORT :246:hist:mont:year') 
         call metadata_edio(nvar,igr,'No metadata available','[NA]','NA') 
      end if
      ...
      if (associated(cpoly%agb_cut)) then
         nvar=nvar+1
           ...
           var_len,var_len_global,max_ptrs,'AGB_CUT :246:hist:mont:year') 
         call metadata_edio(nvar,igr,'No metadata available','[NA]','NA') 
      end if
   ...
   subroutine filltab_sitetype(igr,ipy,isi,init)
      ...
      if (associated(csite%fast_soil_C)) then
         nvar=nvar+1
           ...
           var_len,var_len_global,max_ptrs,'FAST_SOIL_C :31:hist:mont:year') 
         call metadata_edio(nvar,igr,'No metadata available','[NA]','NA') 
      end if
      ...
      if (associated(csite%slow_soil_C)) then
         nvar=nvar+1
           ...
           var_len,var_len_global,max_ptrs,'SLOW_SOIL_C :31:hist:mont:year') 
         call metadata_edio(nvar,igr,'No metadata available','[NA]','NA') 
      end if
      ...
      if (associated(csite%structural_soil_C)) then
         nvar=nvar+1
           ...
           var_len,var_len_global,max_ptrs,'STRUCTURAL_SOIL_C :31:hist:mont:year') 
         call metadata_edio(nvar,igr,'No metadata available','[NA]','NA') 
      end if
      ...
      if (associated(csite%mineralized_soil_N)) then
         nvar=nvar+1
           ...
           var_len,var_len_global,max_ptrs,'MINERALIZED_SOIL_N :31:hist:mont:year') 
         call metadata_edio(nvar,igr,'No metadata available','[NA]','NA') 
      end if
      ...
      if (associated(csite%slow_soil_N)) then
         nvar=nvar+1
           call vtable_edio_r(npts,csite%slow_soil_N,nvar,igr,init,csite%paglob_id, &
      ...
      if (associated(csite%sum_cdd)) then
         nvar=nvar+1
           call vtable_edio_r(npts,csite%sum_cdd,nvar,igr,init,csite%paglob_id, &
           var_len,var_len_global,max_ptrs,'SUM_CDD :31:hist') 
         call metadata_edio(nvar,igr,'No metadata available','[NA]','NA') 
      end if
      ...
      if (associated(csite%mean_rh)) then
         nvar=nvar+1
           ...
           var_len,var_len_global,max_ptrs,'MEAN_RH :31:hist:anal') 
      ...
      if (associated(csite%dmean_nutrient_layer_drainage)) then
         nvar=nvar+1
           call vtable_edio_r(npts,csite%dmean_nutrient_layer_drainage,nvar,igr,init,csite%paglob_id, &
           var_len,var_len_global,max_ptrs,'DMEAN_NUTRIENT_LAYER_DRAINAGE :31:hist:dail') 
         call metadata_edio(nvar,igr,'Daily mean of drainage of nutrient layer','[---]','ipatch') 
      end if
      ...
      if (associated(csite%mean_nep)) then
         nvar=nvar+1
           ...
           var_len,var_len_global,max_ptrs,'MEAN_NEP :31:hist:anal') 
         call metadata_edio(nvar,igr,'No metadata available','[NA]','NA') 
      end if
      ...
      if (associated(csite%green_leaf_factor)) then
         nvar=nvar+1
           call vtable_edio_r(npts,csite%green_leaf_factor,nvar,igr,init,csite%paglob_id, &
           var_len,var_len_global,max_ptrs,'GREEN_LEAF_FACTOR_PA :34:hist') 
         call metadata_edio(nvar,igr,'No metadata available','[NA]','NA') 
      end if

      if (associated(csite%leaf_aging_factor)) then
         nvar=nvar+1
           call vtable_edio_r(npts,csite%leaf_aging_factor,nvar,igr,init,csite%paglob_id, &
           var_len,var_len_global,max_ptrs,'LEAF_AGING_FACTOR_PA :34:hist') 
         call metadata_edio(nvar,igr,'No metadata available','[NA]','NA') 
      end if

      if (associated(csite%phenophase)) then
         nvar=nvar+1
           call vtable_edio_i(npts,csite%phenophase,nvar,igr,init,csite%paglob_id, &
           var_len,var_len_global,max_ptrs,'PHENOPHASE :34:hist') 
         call metadata_edio(nvar,igr,'NA','[NA]','NA') 
      end if
      ...
      if (associated(csite%slsc_in)) then
         nvar=nvar+1
           call vtable_edio_r(npts,csite%slsc_in,nvar,igr,init,csite%paglob_id, &
           var_len,var_len_global,max_ptrs,'SLSC_IN :31:hist') 
         call metadata_edio(nvar,igr,'No metadata available','[NA]','NA') 
      end if
      ...
      if (associated(csite%slsn_in)) then
         nvar=nvar+1
           call vtable_edio_r(npts,csite%slsn_in,nvar,igr,init,csite%paglob_id, &
           var_len,var_len_global,max_ptrs,'SLSN_IN :31:hist') 
         call metadata_edio(nvar,igr,'No metadata available','[NA]','NA') 
      end if
...
!ATT
      if (associated(csite%ntext_soil)) then
         nvar=nvar+1
           ...
           var_len,var_len_global,max_ptrs,'NTEXT_SOIL_PA :32:hist:mont') 
         call metadata_edio(nvar,igr,'No metadata available','[NA]','NA') 
      end if
!ATT
      ...
      npts = csite%npatches * nzg * 31

      if (associated(csite%past_psi)) then
         nvar=nvar+1
         call vtable_edio_r(npts,csite%past_psi,nvar,igr,init,csite%paglob_id, &
              var_len,var_len_global,max_ptrs,'PAST_PSI_PA :318:hist:mont:dail') 
         call metadata_edio(nvar,igr,'Past 31 day soil water potential','[NA]','NA') 
      end if
      !------------------------------------------------------------------------------------!
   ...
   subroutine filltab_patchtype(igr,ipy,isi,ipa,init)
      ...
      if (associated(cpatch%krdepth)) then
         nvar=nvar+1
           ...
           var_len,var_len_global,max_ptrs,'KRDEPTH :40:hist:mont:anal') 
         call metadata_edio(nvar,igr,'No metadata available','[NA]','NA') 
      end if
      ...
      if (associated(cpatch%bstorage_min)) then
         nvar=nvar+1
           call vtable_edio_r(npts,cpatch%bstorage_min,nvar,igr,init,cpatch%coglob_id, &
           var_len,var_len_global,max_ptrs,'BSTORAGE_MIN :41:hist:year:mont') 
         call metadata_edio(nvar,igr,'No metadata available','[NA]','NA') 
      end if
     ...
     if (associated(cpatch%nstorage_min)) then
         nvar=nvar+1
           call vtable_edio_r(npts,cpatch%nstorage_min,nvar,igr,init,cpatch%coglob_id, &
           var_len,var_len_global,max_ptrs,'NSTORAGE_MIN :41:hist:year:mont') 
         call metadata_edio(nvar,igr,'No metadata available','[NA]','NA') 
      end if
...
!JL!
      ...
      if (associated(cpatch%m_coef)) then
         nvar=nvar+1
           call vtable_edio_r(npts,cpatch%m_coef,nvar,igr,init,cpatch%coglob_id, &
           var_len,var_len_global,max_ptrs,'M_COEF :41:hist') 
         call metadata_edio(nvar,igr,'No metadata available','[NA]','NA') 
      end if

      if (associated(cpatch%former_gJ)) then
         nvar=nvar+1
           call vtable_edio_r(npts,cpatch%former_gJ,nvar,igr,init,cpatch%coglob_id, &
           var_len,var_len_global,max_ptrs,'FORMER_gV :41:hist') 
         call metadata_edio(nvar,igr,'No metadata available','[NA]','NA') 
      end if

      if (associated(cpatch%former_gV)) then
         nvar=nvar+1
           call vtable_edio_r(npts,cpatch%former_gV,nvar,igr,init,cpatch%coglob_id, &
           var_len,var_len_global,max_ptrs,'FORMER_GV :41:hist') 
         call metadata_edio(nvar,igr,'No metadata available','[NA]','NA') 
      end if

      if (associated(cpatch%avg_psi_leaf)) then
         nvar=nvar+1
           call vtable_edio_r(npts,cpatch%avg_psi_leaf,nvar,igr,init,cpatch%coglob_id, &
           var_len,var_len_global,max_ptrs,'AVG_PSI_LEAF :41:hist:anal') 
         call metadata_edio(nvar,igr,'No metadata available','[NA]','NA') 
      end if

      if (associated(cpatch%avg_psi_stem)) then
         nvar=nvar+1
           call vtable_edio_r(npts,cpatch%avg_psi_stem,nvar,igr,init,cpatch%coglob_id, &
           var_len,var_len_global,max_ptrs,'AVG_PSI_STEM :41:hist:anal') 
         call metadata_edio(nvar,igr,'No metadata available','[NA]','NA') 
	  endif

      if (associated(cpatch%avg_m_coef)) then
         nvar=nvar+1
           call vtable_edio_r(npts,cpatch%avg_m_coef,nvar,igr,init,cpatch%coglob_id, &
           var_len,var_len_global,max_ptrs,'AVG_M_COEF :41:hist:anal') 
         call metadata_edio(nvar,igr,'No metadata available','[NA]','NA') 
	  endif

      if (associated(cpatch%avg_rl_flow)) then
         nvar=nvar+1
           call vtable_edio_r(npts,cpatch%avg_rl_flow,nvar,igr,init,cpatch%coglob_id, &
           var_len,var_len_global,max_ptrs,'AVG_RL_FLOW :41:hist:anal') 
         call metadata_edio(nvar,igr,'No metadata available','[NA]','NA') 
	  endif

      if (associated(cpatch%avg_sr_flow)) then
         nvar=nvar+1
           call vtable_edio_r(npts,cpatch%avg_sr_flow,nvar,igr,init,cpatch%coglob_id, &
           var_len,var_len_global,max_ptrs,'AVG_SR_FLOW :41:hist:anal') 
         call metadata_edio(nvar,igr,'No metadata available','[NA]','NA') 
	  endif

      if (associated(cpatch%hite_coef)) then
         nvar=nvar+1
           call vtable_edio_r(npts,cpatch%hite_coef,nvar,igr,init,cpatch%coglob_id, &
           var_len,var_len_global,max_ptrs,'HITE_COEF :41:hist:mont') 
         call metadata_edio(nvar,igr,'No metadata available','[NA]','NA') 
      end if

      if (associated(cpatch%rl_flow)) then
         nvar=nvar+1
           call vtable_edio_r(npts,cpatch%rl_flow,nvar,igr,init,cpatch%coglob_id, &
           var_len,var_len_global,max_ptrs,'RL_FLOW :41:hist') 
         call metadata_edio(nvar,igr,'No metadata available','[NA]','NA') 
      end if

      if (associated(cpatch%sr_flow)) then
         nvar=nvar+1
           call vtable_edio_r(npts,cpatch%sr_flow,nvar,igr,init,cpatch%coglob_id, &
           var_len,var_len_global,max_ptrs,'SR_FLOW :41:hist') 
         call metadata_edio(nvar,igr,'No metadata available','[NA]','NA') 
      end if
      ...
      if (associated(cpatch%predawn_psi_leaf)) then
         nvar=nvar+1
           call vtable_edio_r(npts,cpatch%predawn_psi_leaf,nvar,igr,init,cpatch%coglob_id, &
           var_len,var_len_global,max_ptrs,'PREDAWN_PSI_LEAF :41:dail:hist') 
         call metadata_edio(nvar,igr,'No metadata available','[NA]','NA') 
      end if

      if (associated(cpatch%midday_psi_leaf)) then
         nvar=nvar+1
           call vtable_edio_r(npts,cpatch%midday_psi_leaf,nvar,igr,init,cpatch%coglob_id, &
           var_len,var_len_global,max_ptrs,'MIDDAY_PSI_LEAF :41:dail:hist') 
         call metadata_edio(nvar,igr,'No metadata available','[NA]','NA') 
      end if

      if (associated(cpatch%predawn_psi_stem)) then
         nvar=nvar+1
           call vtable_edio_r(npts,cpatch%predawn_psi_stem,nvar,igr,init,cpatch%coglob_id, &
           var_len,var_len_global,max_ptrs,'PREDAWN_PSI_STEM :41:dail:hist') 
         call metadata_edio(nvar,igr,'No metadata available','[NA]','NA') 
      end if

      if (associated(cpatch%midday_psi_stem)) then
         nvar=nvar+1
           call vtable_edio_r(npts,cpatch%midday_psi_stem,nvar,igr,init,cpatch%coglob_id, &
           var_len,var_len_global,max_ptrs,'MIDDAY_PSI_STEM :41:dail:hist') 
         call metadata_edio(nvar,igr,'No metadata available','[NA]','NA') 
      end if
      ...
      ! XXT for the phenology of tropical deciduous forest
      ! use type 50
      npts = cpatch%ncohorts * 31
      
      
      if (associated(cpatch%past_psi_leaf)) then
         nvar=nvar+1
           call vtable_edio_r(npts,cpatch%past_psi_leaf,nvar,igr,init,cpatch%coglob_id, &
           var_len,var_len_global,max_ptrs,'PAST_PSI_LEAF_CO :50:hist:dail') 
         call metadata_edio(nvar,igr,'Past month leaf psi','','31 - icohort') 
      end if



      ! water supply layer, ncohorts * nzg, use 51
      npts = cpatch%ncohorts * nzg
      
      if (associated(cpatch%water_supply_layer_frac)) then
         nvar=nvar+1
         call vtable_edio_r(npts,cpatch%water_supply_layer_frac,nvar,igr,init,cpatch%coglob_id, &
              var_len,var_len_global,max_ptrs,'WATER_SUPPLY_LAYER_FRAC :51:hist')
         call metadata_edio(nvar,igr,'No metadata available','[NA]','NA')
      end if
      !------------------------------------------------------------------------------------!
      !------------------------------------------------------------------------------------!
