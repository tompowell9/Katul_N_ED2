...
module rk4_coms
   ...
   type rk4patchtype
      ...
      real(kind=8)                      :: avg_nutrient_layer_drainage      ! Drainage at the nutrient layer.
   ...
   subroutine find_derived_thbounds(can_rhos,can_theta,can_temp,can_shv,can_rvap,can_prss  &
      ...
      type(sitetype)              , target     :: csite!ATT
      integer,  dimension(nzg)   , intent(in) :: ntext_soil !ATT
      !----- Local variables. -------------------------------------------------------------!
     ...
     ! rk4max_can_theiv = max(rk4max_can_theiv,can_theiv_try)
       rk4max_can_theiv = max(rk4max_can_theiv,9.0E+02)
      !----- 3. Minimum pressure. ---------------------------------------------------------!
      can_theta_try    = can_temp * (p008 / rk4min_can_prss) ** rocp8
