   ...
   subroutine terminate_cohorts(csite,ipa,elim_nplant,elim_lai)
      ...
      do ico = 1,cpatch%ncohorts
         ...
         if ( csize < min_cohort_size(ipft) ) then
            ...
            !----- Update litter pools ----------------------------------------------------!
            !If the plant is black spruce (PFT 22), channel the nitrogen and carbon and 
            !litter into the slsc and slsn pools. All other PFT litter goes to the fsc 
            !and fsn litter pools

            csite%slsc_in(ipa) = csite%slsc_in(ipa) + cpatch%nplant(ico) * (1-f_fast(ipft))&
                              * (f_labile(ipft)*cpatch%balive(ico) + cpatch%bstorage(ico))
            csite%slsn_in(ipa) = csite%slsn_in(ipa) + cpatch%nplant(ico) * (1-f_fast(ipft))&
                              * (f_labile(ipft) * cpatch%balive(ico) / c2n_leaf(ipft)      &
                                + cpatch%nstorage(ico))          
            csite%fsc_in(ipa) = csite%fsc_in(ipa) + cpatch%nplant(ico) * f_fast(ipft)      &
                              * (f_labile(ipft)*cpatch%balive(ico) + cpatch%bstorage(ico))
            csite%fsn_in(ipa) = csite%fsn_in(ipa) + cpatch%nplant(ico) * f_fast(ipft)      &
                              * ( f_labile(ipft) * cpatch%balive(ico) / c2n_leaf(ipft)     &
                                + cpatch%nstorage(ico))
...
!              * cpatch%sla(ico) + cpatch%wai(ico)
		  stai = cpatch%nplant(ico) * dbh2bl(cpatch%dbh(ico),ipft) *  &
			  cpatch%sla(ico) + cpatch%wai(ico)
   ...
   subroutine clone_cohort(cpatch,isc,idt)
      ...
      cpatch%bstorage_min(idt)         = cpatch%bstorage_min(isc)
      cpatch%nstorage(idt)             = cpatch%nstorage(isc)
      cpatch%nstorage_min(idt)         = cpatch%nstorage_min(isc)
      ...
      enddo
      cpatch%cohort_id(idt)            = cpatch%cohort_id(isc)
      ...
      cpatch%psi_leaf(idt)             = cpatch%psi_leaf(isc)
      cpatch%psi_stem(idt)             = cpatch%psi_stem(isc)
      
	  cpatch%rl_flow(idt)              = cpatch%rl_flow(isc)
	  cpatch%sr_flow(idt)              = cpatch%sr_flow(isc)

	  cpatch%hite_coef(idt)			   = cpatch%hite_coef(isc)
	  cpatch%m_coef(idt)			   = cpatch%m_coef(isc)
	  cpatch%former_gJ(idt)			   = cpatch%former_gJ(isc)
	  cpatch%former_gV(idt)			   = cpatch%former_gV(isc)

	  cpatch%midday_psi_leaf(idt)      = cpatch%midday_psi_leaf(isc)
	  cpatch%predawn_psi_leaf(idt)     = cpatch%predawn_psi_leaf(isc)

	  cpatch%past_psi_leaf(:,idt)      = cpatch%past_psi_leaf(:,isc)
	  cpatch%water_supply_layer_frac(:,idt)      =  &
		cpatch%water_supply_layer_frac(:,isc)
	  ...
	  if (ifoutput > 0) then
	  	cpatch%avg_psi_stem(idt) = cpatch%avg_psi_stem(isc)	
	  	cpatch%avg_psi_leaf(idt) = cpatch%avg_psi_leaf(isc)	
	  	cpatch%avg_rl_flow(idt) = cpatch%avg_rl_flow(isc)	
	  	cpatch%avg_sr_flow(idt) = cpatch%avg_sr_flow(isc)	
	  	cpatch%avg_m_coef(idt) = cpatch%avg_m_coef(isc)	
	  endif
   ...
   subroutine fuse_2_cohorts(cpatch,donc,recc, newn,green_leaf_factor, can_prss,lsl)
      ...
      cpatch%bstorage_min(recc)  = ( cpatch%nplant(recc) * cpatch%bstorage_min(recc)               &
                               + cpatch%nplant(donc) * cpatch%bstorage_min(donc) ) * newni

      cpatch%nstorage(recc)  = ( cpatch%nplant(recc) * cpatch%nstorage(recc)               &
                               + cpatch%nplant(donc) * cpatch%nstorage(donc) ) * newni

      cpatch%nstorage_min(recc)  = ( cpatch%nplant(recc) * cpatch%nstorage_min(recc)               &
                               + cpatch%nplant(donc) * cpatch%nstorage_min(donc) ) * newni
	  ...
	  ! update psi_leaf, psi_stem, past_psi_leaf
	  ! there's no need to update sapflow or water supply, which will be updated
	  ! from psi_leaf and psi_stem

      cpatch%psi_leaf(recc)       = ( cpatch%psi_leaf(recc)   * cpatch%lai(recc)               &
                                  + cpatch%psi_leaf(donc)   * cpatch%lai(donc) ) * newlaii
      cpatch%past_psi_leaf(:,recc)       = ( cpatch%past_psi_leaf(:,recc)   * cpatch%lai(recc)               &
                                  + cpatch%past_psi_leaf(:,donc)   * cpatch%lai(donc) ) * newlaii
      cpatch%psi_stem(recc)       = ( cpatch%psi_stem(recc)   * cpatch%bsapwood(recc) * cpatch%nplant(recc) &
                                  + cpatch%psi_stem(donc)   * cpatch%bsapwood(donc) * cpatch%nplant(donc) ) / &
	  							(cpatch%bsapwood(recc) * cpatch%nplant(recc) +  cpatch%bsapwood(donc) * cpatch%nplant(donc))
      !------------------------------------------------------------------------------------!
         ...
         siteloop: do isi = 1,cpoly%nsites
            ...
            donloope: do donp=csite%npatches,2,-1
               ...
               recloope: do recp=donp-1,1,-1
                  ...
                  call fuse_2_patches(csite,donp,recp,nzg,nzs,cpoly%met(isi)%prss        &
                                     ,cpoly%lsl(isi),csite%ntext_soil(:,donp),csite%ntext_soil(:,recp)               &
                                     ,csite%green_leaf_factor(:,donp),elim_nplant,elim_lai)
            ...
            mainfuseloop: do ifus=0,niter_patfus
               ...
               donloopp: do donp = csite%npatches,2,-1
                  ...
                  ! recpatch.                                                              !
                  !------------------------------------------------------------------------!
                  !ATT, take soil properties of largest patch
                  call fuse_2_patches(csite,donp,recp,nzg,nzs,cpoly%met(isi)%prss        &
                                     ,cpoly%lsl(isi),csite%ntext_soil(:,donp),csite%ntext_soil(:,recp)               &
                                     ,csite%green_leaf_factor(:,donp),elim_nplant,elim_lai)
   ...
   !---------------------------------------------------------------------------------------!
   subroutine fuse_2_patches(csite,donp,recp,mzg,mzs,prss,lsl,ntext_soil_donp,ntext_soil_recp,green_leaf_factor &
      ...
      integer, dimension(mzg), intent(in)  :: ntext_soil_donp   ! Soil type
      integer, dimension(mzg), intent(in)  :: ntext_soil_recp   ! Soil type
      ...
      integer, dimension(mzg)              :: ntext_soil        ! Soil type
      ...
      csite%slow_soil_N(recp)        = newareai *                                          &!ATT
                                     ( csite%slow_soil_N(donp)        * csite%area(donp)   &
                                     + csite%slow_soil_N(recp)        * csite%area(recp) )    
      ...
      !ATT assign soil type of larger patch
      csite%ntext_soil(:,recp) = ntext_soil_donp
      ntext_soil = ntext_soil_donp
      !end ATT
      do iii=1,mzg
      ...
      csite%avg_nutrient_layer_drainage(recp)        = newareai *                                         &
                                      ( csite%avg_nutrient_layer_drainage(donp)      * csite%area(donp)   &
      ...
      !------------------------------------------------------------------------------------! 
         csite%dmean_nutrient_layer_drainage(recp)         = newareai                                      &
                                           * ( csite%dmean_nutrient_layer_drainage(donp)                   &
                                             * csite%area(donp)                            &
	...
	!JL!
	 csite%mmean_total_plant_nitrogen_uptake(recp)       = newareai                                         &
      ...
      ! + csite%can_rhos(recp)                                                             !
      !------------------------------------------------------------------------------------!
      call update_patch_thermo_props(csite,recp,recp,mzg,mzs)
