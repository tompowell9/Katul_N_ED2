...
!            csite%avg_nutrient_layer_drainage       (ipa) = csite%avg_nutrient_layer_drainage       (ipa) * frqsumi
            ...
            do ico=1,cpatch%ncohorts
			   ...
			   !---   update vars related to water use
			   cpatch%avg_psi_stem(ico)  = cpatch%avg_psi_stem(ico) * tfact
			   cpatch%avg_psi_leaf(ico)  = cpatch%avg_psi_leaf(ico) * tfact
			   cpatch%avg_rl_flow(ico)  = cpatch%avg_rl_flow(ico) * tfact
			   cpatch%avg_sr_flow(ico)  = cpatch%avg_sr_flow(ico) * tfact
			   cpatch%avg_m_coef(ico)  = cpatch%avg_m_coef(ico) * tfact
            end do
...
!            csite%avg_nutrient_layer_drainage(ipa)         = 0.0
            ...
            cohortloop: do ico=1,cpatch%ncohorts
               ...
               cpatch%avg_psi_stem(ico)       = 0.0
               cpatch%avg_psi_leaf(ico)       = 0.0
               cpatch%avg_rl_flow(ico)       = 0.0
               cpatch%avg_sr_flow(ico)       = 0.0
               cpatch%avg_m_coef(ico)       = 0.0
   ...
   polyloop: do ipy=1,cgrid%npolygons
      ...
      siteloop: do isi=1,cpoly%nsites
         ...
         patchloop: do ipa=1,csite%npatches
            ...
            !------------------------------------------------------------------------------!
            csite%dmean_albedo(ipa)              = csite%dmean_albedo(ipa) * dtlsm_o_daylight !ATT
            csite%dmean_albedo_beam(ipa)         = csite%dmean_albedo_beam(ipa) * dtlsm_o_daylight
            csite%dmean_albedo_diffuse(ipa)      = csite%dmean_albedo_diffuse(ipa) * dtlsm_o_daylight
            !------------------------------------------------------------------------------!
...
subroutine normalize_daily_vars(cgrid)
   use grid_coms    , only : nzg           ! ! intent(in)
   use ed_misc_coms , only : radfrq        & ! intent(in)
                           , current_time  & ! ! intent(in)
   							, dtlsm       & ! intent(in)
   ...
   real                           :: tfact
   real                           :: frqsumi
   real  						   :: dtlsm_o_hoursec
   !---------------------------------------------------------------------------------------!
   dtlsm_o_hoursec   = dtlsm / 3600.
      ...
      do isi = 1,cpoly%nsites
         ...
         do ipa = 1,csite%npatches
            ...
            csite%dmean_nutrient_layer_drainage(ipa)         = csite%dmean_nutrient_layer_drainage(ipa)                    &
                                             / day_sec
            do ico=1,cpatch%ncohorts
			    ! update predawn, midday and past leaf_psi
			   cpatch%predawn_psi_leaf  (ico) = cpatch%predawn_psi_leaf  (ico)			   &
			   								  * dtlsm_o_hoursec
			   cpatch%midday_psi_leaf  (ico) = cpatch%midday_psi_leaf  (ico)			   &
			   								  * dtlsm_o_hoursec
			   
			   cpatch%past_psi_leaf(31,ico) = cpatch%predawn_psi_leaf(ico)

				do iday = 1,30
					cpatch%past_psi_leaf(iday,ico) = cpatch%past_psi_leaf(iday+1,ico)
				enddo
!print*,'predawn',cpatch%predawn_psi_leaf(ico),'midday',cpatch%midday_psi_leaf(ico)
!print*,'past_psi_leaf',cpatch%past_psi_leaf
            end do
         end do
      end do
   end do
   ...
   return
end subroutine normalize_daily_vars



subroutine reset_daily_vars(cgrid)
   ...
   cpoly => cgrid%polygon(ipy)
      siteloop: do isi = 1,cpoly%nsites
		...
		patchloop: do ipa = 1,csite%npatches
            ...
            cohortloop: do ico=1,cpatch%ncohorts
				cpatch%predawn_psi_leaf(ico) = 0.
				cpatch%midday_psi_leaf(ico) = 0.
				cpatch%past_psi_leaf(31,ico) = 0.
            end do cohortloop
         end do patchloop
      end do siteloop
   end do polyloop
   ...
   return
end subroutine reset_daily_vars
!==========================================================================================!
!==========================================================================================!
