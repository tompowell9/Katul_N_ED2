...
module phenology_startup
   ...
   contains
     ...
     subroutine initialize_paw(csite,ipa,ntext_soil)
       use grid_coms, only: nzg
       use ed_state_vars, only: sitetype
       use soil_coms, only: soil, slz
       implicit none

       real, dimension(nzg) :: relative_porosity, weighted_relative_porosity,  &
       ...
       do k = 1, nzg
         ! nsoil = ntext_soil(k)
          ...
          relative_porosity(k) = max(0.d0,(csite%soil_water(nzg,ipa) - soil(nsoil)%soilwp))    &
               / (soil(nsoil)%slmsts -soil(nsoil)%soilwp) 
          if(k < nzg)then
             weighted_relative_porosity(k) = relative_porosity(k) * (slz(k+1)-slz(k))
          else
             weighted_relative_porosity(k) = relative_porosity(k) * (-slz(k))
          endif
       enddo
       ...
       do k = 1, nzg
          avg_rel_porosity(k) = cumul_wei_rel_porosity(k) / (-slz(k))
       enddo
       
       ! Update running average of plant available water.
       do k = 1, nzg
          csite%current_paw(k,ipa) = 0.0
          csite%past_paw(k,1:10,ipa) = avg_rel_porosity(k)
       enddo



       return
     end subroutine initialize_paw
...
print*,'read_prescribed_phenology is broken'
stop
