...
subroutine init_ed_cohort_vars(cpatch,ico, lsl)
   ...
   !---------------------------------------------------------------------------------------!
   if (ifoutput > 0) then
   	cpatch%avg_psi_leaf(ico)		= 0.
   	cpatch%avg_psi_stem(ico)		= 0.
   	cpatch%avg_rl_flow(ico)		= 0.
   	cpatch%avg_sr_flow(ico)		= 0.
   	cpatch%avg_m_coef(ico)		= 0.
   endif
...
subroutine init_ed_patch_vars(csite,ip1,ip2,lsl)
   ...
   do ipft = 1, n_pft
      if(phenology(ipft) == 2)then
         csite%green_leaf_factor(ipft,ip1:ip2) = 1.0
      else
         csite%green_leaf_factor(ipft,ip1:ip2) = 1.0
      endif
   enddo
   csite%leaf_aging_factor(1:n_pft,ip1:ip2) = 1.0
   ...
   csite%ebudget_residual(ip1:ip2)         = 0.0
      ...
      csite%dmean_nutrient_layer_drainage         (ip1:ip2) = 0.0
   ...
   csite%phenophase(1:n_pft,ip1:ip2) = 1
   ...
   csite%avg_nutrient_layer_drainage         (ip1:ip2) = 0.0
